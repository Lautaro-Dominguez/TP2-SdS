...
Para cada (densidad, eta) corre --seeds semillas independientes (genera partículas +
simula desde cero para cada una, via sim.app.GenerateParticlesMain / SimulateMain sin
tocarlos). Para cada semilla promedia va(t) desde --t-start (el inicio del estacionario,
determinado antes con va_vs_t.py) hasta el final de la corrida, y despues promedia esos
escalares entre semillas: el desvio estandar entre semillas es la barra de error. Esto
combina promediado temporal en el estacionario y promediado entre realizaciones
independientes, que es el criterio mas preciso de los tres que se evaluaron.
...
def run_one(density, l, eta, model, rc, speed, iterations, seed, workdir):
    tag = f"rho{density}_eta{eta:.4f}_seed{seed}"
    traj = workdir / f"traj_{tag}.txt"
    clusters = workdir / f"clusters_{tag}.txt"
    timing = workdir / f"timing_{tag}.txt"
    generate_particles(l, density, seed, traj)
    simulate(eta, iterations, model, rc, l, True, speed, seed, traj, traj, clusters, timing)
...
def main():
    ...
    parser.add_argument("--seeds", type=int, default=5,
                         help="cantidad de realizaciones independientes por (rho, eta)")
